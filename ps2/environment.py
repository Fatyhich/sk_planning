...
class ManipulatorEnv:
    ...
    def check_collision(self, state_to_check: State) -> (bool, dict):
        """
        Checks state (configuration) for collisions.
        :param state_to_check: State to check for collisions.
        :return: Tuple (collision_detected, collision_info).
                 collision_detected: True if there is a collision, False otherwise.
                 collision_info: A dictionary containing details of the collision (obstacle index, link segment, and collision point).
                 If no collision is detected, collision_info will be None.
        """
        for obs_index, obs in enumerate(self._obstacles):
            for i in range(ManipulatorEnv.N_LINKS):
                ...
                # Check if p1 or p2 is in circle
                if np.linalg.norm(p0 - p1) <= r:
                    return True, {"obstacle_index": obs_index, "link_segment": (i, i+1), "collision_point": p1.tolist()}
                if np.linalg.norm(p0 - p2) <= r:
                    return True, {"obstacle_index": obs_index, "link_segment": (i, i+1), "collision_point": p2.tolist()}
                ...
                if np.linalg.norm(p0 - p4) <= r:
                    return True, {"obstacle_index": obs_index, "link_segment": (i, i+1), "collision_point": p4.tolist()}
